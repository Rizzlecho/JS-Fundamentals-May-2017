        ...
        if (command === 'restock') {
            ...
            for (let microElement in currentProductStats) {
                if (currentProductStats.hasOwnProperty(microElement)) {
                    let microElQuantity = currentProductStats[microElement];
                    if (robot[microElement] < microElQuantity * selectedProductQuantity) {
            ...
            if (canProductBeCooked) {
                for (let microElement in currentProductStats) {
                    if (currentProductStats.hasOwnProperty(microElement)) {
                        let microElQuantity = currentProductStats[microElement];
                        robot[microElement] -=microElQuantity * selectedProductQuantity;
                    }
                }
